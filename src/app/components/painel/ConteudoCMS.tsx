...
export default function ConteudoCMS({
  ...
  const [subTab, setSubTab] = useState<'eventos' | 'noticias' | 'cursos' | 'instrumentos' | 'galeria'>('eventos');
      ...
      <div className="pb-4 border-b border-neutral-850 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex bg-neutral-900 border border-neutral-800 p-0.5 rounded-lg text-xs">
          ...
          <button
            ...
            className={`p-1.5 px-3 rounded-md font-semibold cursor-pointer transition-all ${subTab === 'instrumentos' ? 'bg-[#0B4DA2] text-white shadow' : 'text-neutral-400'}`}
          >
            Instrumentos
          </button>
          <button
      ...
      {subTab === 'cursos' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {coursesLoading ? (
              <div className="col-span-2 text-center py-12 text-xs text-neutral-500 font-mono">Carregando oficinas...</div>
            ) : courses.length === 0 ? (
              <div className="col-span-2 text-center py-12 text-xs text-neutral-500 font-mono border border-dashed border-neutral-800 rounded-xl">
              ...
              <div key={crs.id} className="p-4 rounded-xl bg-neutral-900 border border-neutral-850 flex gap-4 justify-between items-start">
                ...
                <div className="flex flex-col gap-1.5 self-center">
                  <button
                    ...
                    className="p-1 px-2.5 bg-neutral-850 hover:bg-neutral-800 text-amber-500 border border-neutral-800 rounded font-mono text-[9px] cursor-pointer"
                  >
                    EDITAR
                  </button>
                  <button
                    ...
                    className="p-1 px-2.5 bg-neutral-850 hover:bg-rose-950 text-rose-500 border border-neutral-800 rounded font-mono text-[9px] cursor-pointer"
                  >
                    EXCLUIR
                  </button>
                </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center bg-neutral-900 border border-neutral-800 p-4 rounded-xl">
            <div>
              ...
              <p className="text-xs text-neutral-400">Cadastre os instrumentos da orquestra com descrição, galeria e vídeo de demonstração.</p>
            </div>
            <button
              type="button"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xxl:grid-cols-3 gap-6">
            {instrumentsLoading ? (
              <div className="col-span-3 text-center py-12 text-xs text-neutral-500 font-mono">Carregando instrumentos...</div>
            ) : instruments.length === 0 ? (
              <div className="col-span-3 text-center py-12 text-xs text-neutral-500 font-mono border border-dashed border-neutral-800 rounded-xl">
              ...
              >
                <div className="relative">
                  ...
                  <span 
                    ...
                    style={{ backgroundColor: inst.color || '#F2C94C' }}
                  >
                    {inst.slug}
                  </span>
                  {inst.gallery?.length > 0 && (
                    <span className="absolute top-2 right-2 bg-black/75 backdrop-blur-sm text-neutral-200 text-[9px] font-mono font-bold p-1 px-2.5 rounded-full border border-neutral-800">
                ...
                <div className="p-4 flex-1 space-y-2">
                  <h4 className="text-xs font-bold text-neutral-100 font-sans tracking-tight leading-snug">{inst.name}</h4>
                  <p className="text-[11px] text-neutral-400 line-clamp-2">{inst.description}</p>
                  {inst.videoUrl && (
                ...
                <div className="p-2.5 bg-neutral-955 border-t border-neutral-900 flex items-center justify-end space-x-1 text-xs">
                  <button
                    type="button"
      ...
      {subTab === 'galeria' && (
        <div className="space-y-4">
          ...
          <div className="flex justify-between items-center bg-neutral-900 border border-neutral-800 p-4 rounded-xl flex-wrap gap-2.5">
            <div className="flex space-x-1.5 bg-neutral-950 p-0.5 rounded border border-neutral-800 text-xs">
              ...
              <button
                ...
                className={`p-1 px-3 rounded font-mono font-bold cursor-pointer text-[10px] ${galleryMode === 'midia' ? 'bg-[#0B4DA2] text-white' : 'text-neutral-500'}`}
              >
                BIBLIOTECA DE MÍDIA
              </button>
            </div>
            ...
            {galleryMode === 'midia' && (
              <button
                ...
                className="p-1.5 px-3 bg-neutral-800 hover:bg-neutral-750 text-neutral-300 text-xs font-mono font-bold rounded disabled:opacity-60"
              >
          ...
          {galleryMode === 'midia' && (
            <div>
              {mediaLoading ? (
                <div className="text-center py-12 text-xs text-neutral-500 font-mono">Carregando arquivos do Storage...</div>
              ) : mediaFiles.length === 0 ? (
                <div className="text-center py-12 text-xs text-neutral-500 font-mono border border-dashed border-neutral-800 rounded-xl">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {mediaFiles.map((file) => (
                    <div key={file.path} className="relative rounded-lg overflow-hidden border border-neutral-850 bg-neutral-900 group">
                      <img 
                        src={file.url} 
                        alt={file.name} 
                        referrerPolicy="no-referrer"
                        className="w-full h-32 object-cover bg-neutral-950" 
                      />
                      <span className="absolute top-1.5 left-1.5 bg-black/75 backdrop-blur-sm text-amber-500 text-[8px] font-mono font-bold uppercase p-1 px-2 rounded-full border border-neutral-800">
                        {file.folder}
                      </span>

                      <div className="absolute inset-0 bg-black/85 opacity-0 group-hover:opacity-100 transition-all p-3 flex flex-col justify-between text-[10px] text-neutral-200 leading-tight">
                        ...
                        <div className="flex justify-end">
                          <button 
                            ...
                            className="p-1 px-2 bg-neutral-800 text-rose-500 rounded hover:bg-rose-950 cursor-pointer disabled:opacity-50 flex items-center text-[9px] font-mono font-bold"
                          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 animate-fade-in">
          ...
          >
            <div className="bg-neutral-950 p-4 border-b border-neutral-800 flex justify-between items-center text-xs">
              ...
              <button type="button" onClick={() => setInstrumentModalOpen(false)} className="text-neutral-400">Fechar</button>
            </div>

            <div className="p-4 space-y-3.5 max-h-[460px] overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-mono uppercase text-neutral-500 mb-1">Nome do Instrumento</label>
                  <input 
                    type="text" 
                    required
                ...
                <div>
                  <label className="block text-[10px] font-mono uppercase text-neutral-500 mb-1">Slug (opcional, gerado automaticamente)</label>
                  <input 
                    type="text" 
              ...
              <div>
                <label className="block text-[10px] font-mono uppercase text-neutral-500 mb-1">Descrição Curta</label>
                <textarea 
                  required
              ...
              <div>
                <label className="block text-[10px] font-mono uppercase text-neutral-500 mb-1">Descrição Completa</label>
                <textarea 
                  required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-mono uppercase text-neutral-500 mb-1">Link de Vídeo (YouTube/Vimeo)</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-[10px] font-mono uppercase text-neutral-500 mb-1">Cor de Destaque</label>
                  <input 
                    type="color" 
                    value={activeInstrument.color || '#0B4DA2'}
                    onChange={(e) => setActiveInstrument({ ...activeInstrument, color: e.target.value })}
                    className="w-full bg-neutral-950 border border-neutral-820 h-9 rounded cursor-pointer"
                  />
                </div>
              </div>

              <div className="flex flex-col justify-end">
                <label className="block text-[10px] font-mono uppercase text-neutral-500 mb-2">Imagem Principal</label>
                {activeInstrument.image && (
                  <img
                    src={activeInstrument.image}
                    alt="Pré-visualização"
                    referrerPolicy="no-referrer"
                    className="w-full h-28 object-cover rounded border border-neutral-800 mb-2"
                  />
                )}
                <ImageUploader 
                  allowedTypes="Imagens (.jpg, .png, .webp)"
              ...
              <div className="bg-neutral-955 p-3 rounded-lg border border-neutral-850">
                <label className="block text-[10px] font-mono uppercase text-neutral-500 mb-2 font-bold">Galeria de Fotos</label>
                    ...
                    {activeInstrument.gallery.map((url) => (
                      <div key={url} className="relative group">
                        <img 
                          ...
                          alt="Galeria" 
                          referrerPolicy="no-referrer"
                          className="w-full h-16 object-cover rounded border border-neutral-800" 
                        />
                        <button
                          type="button"
                ...
                <label className="block text-[9px] font-mono uppercase text-neutral-500 mb-1">Adicionar foto à galeria (1 por vez, salva ao confirmar)</label>
                <ImageUploader 
                  allowedTypes="Imagens (.jpg, .png, .webp)"
            ...
            <div className="bg-neutral-950 p-3 border-t border-neutral-800 flex justify-end space-x-2">
              <button type="button" onClick={() => setInstrumentModalOpen(false)} className="text-xs text-neutral-400 px-3 py-1 bg-neutral-900 rounded">Cancelar</button>
              <button 
                type="submit" 
