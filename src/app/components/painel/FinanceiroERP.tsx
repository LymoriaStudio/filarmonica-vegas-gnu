    ...
    <div className="space-y-6 p-6 animate-fade-in select-none">
        ...
        <div className="flex bg-neutral-900 border border-neutral-800 p-0.5 rounded-lg text-xs">
          {(['doacoes', 'apoiadores', 'relatorios'] as const).map((t) => (
            <button
              ...
              type="button"
              onClick={() => setSubTab(t)}
              className={`p-1.5 px-3 rounded-md font-semibold cursor-pointer transition-all ${subTab === t ? 'bg-[#0B4DA2] text-white shadow' : 'text-neutral-400'}`}
            >
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center bg-neutral-900/50 border border-neutral-800 p-4 rounded-xl">
            <div>
              ...
              <p className="text-[11px] text-neutral-400 mt-0.5">Confirme doações para creditar o montante no saldo orquestral.</p>
            </div>
            <div className="flex items-center space-x-2">
              ...
              <button
                ...
                className="p-2 px-3 bg-[#0B4DA2] hover:bg-blue-700 text-white text-xs font-semibold rounded cursor-pointer"
              >
          ...
          {loadingDoacoes && <p className="text-neutral-400 text-sm text-center py-8">Carregando doações...</p>}
          ...
          {!loadingDoacoes && !errorDoacoes && doacoes.length === 0 && (
            <p className="text-neutral-500 text-sm text-center py-8 border border-dashed border-neutral-800 rounded-xl">Nenhuma doação registrada ainda.</p>
          )}
                ...
                <tbody className="divide-y divide-neutral-850">
                  {doacoes.map((don) => (
                    <tr key={don.id} className="hover:bg-neutral-950/40 transition-all font-mono">
                      <td className="p-3 pl-4 text-neutral-500 text-[10px]">{don.id.slice(0, 8)}…</td>
                      <td className="p-3">
                      ...
                      <td className="p-3 text-right pr-4 space-x-1.5 whitespace-nowrap">
                        ...
                        {don.status !== 'cancelado' && (
                          <button
                            ...
                            className="p-1 px-2 bg-neutral-800 hover:bg-rose-950 text-neutral-400 hover:text-rose-200 rounded text-[10px] font-sans cursor-pointer"
                          >
                            Anular
                          </button>
                        )}
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center bg-neutral-900 border border-neutral-800 p-4 rounded-xl">
            <div>
              ...
              <p className="text-[11px] text-neutral-400 mt-0.5">Estipule hierarquias de patrocínio corporativo e configure o site público.</p>
            </div>
            <button type="button" onClick={() => handleOpenSupporterModal(null)} className="p-2 px-3 bg-[#0B4DA2] text-white text-xs font-semibold rounded cursor-pointer">
              + Adicionar Novo Patrocinador
            </button>
          </div>

          {supporters.length === 0 && (
            <p className="text-neutral-500 text-sm text-center py-8 border border-dashed border-neutral-800 rounded-xl">Nenhum patrocinador cadastrado ainda.</p>
          )}
            ...
            {supporters.map((sup) => (
              <div key={sup.id} className="rounded-xl overflow-hidden bg-neutral-900 border border-neutral-800 flex flex-col justify-between">
                <div className="p-4 bg-neutral-950 border-b border-neutral-850 text-center flex flex-col items-center">
                  <img src={sup.logo} alt={sup.name} referrerPolicy="no-referrer" className="w-20 h-10 object-contain rounded mb-3 bg-white/5 p-1 border border-neutral-800" />
                  ...
                  <span className={`inline-block p-1 px-3 mt-2 rounded text-[8.5px] font-extrabold uppercase font-mono tracking-widest ${
                    ...
                    : sup.sponsorshipLevel === 'gold' ? 'bg-amber-900/30 text-[#F2C94C] border border-[#F2C94C]/20'
                ...
                <div className="p-4 flex-1 bg-neutral-900/35 text-[11px] text-neutral-400 leading-snug">
                  ...
                  <div className="mt-3 pt-2.5 border-t border-neutral-850 flex items-center justify-between font-mono text-[9.5px]">
                    ...
                    <a href={sup.siteUrl} target="_blank" rel="noreferrer" className="text-[#F2C94C] hover:underline">Site Corporativo</a>
                  </div>
                </div>
                ...
                <div className="p-2 px-4 bg-neutral-950 border-t border-neutral-850 flex justify-between items-center text-xs">
                  ...
                  <div className="flex space-x-1.5">
                    <button type="button" onClick={() => handleOpenSupporterModal(sup)} className="text-[#F2C94C] hover:text-amber-200 font-mono font-bold text-[10px]">EDITAR</button>
      ...
      {subTab === 'relatorios' && (
        <div className="space-y-6">
          {loadingDoacoes && <p className="text-neutral-400 text-sm text-center py-8">Carregando dados...</p>}

          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {[
              { label: 'Entrada Diária', value: reportTotals.dailyTotal, color: 'text-white', sub: 'Somas do dia corrente' },
              { label: 'Média Mensal', value: reportTotals.monthlyTotal, color: 'text-[#F2C94C]', sub: 'Mês atual' },
              { label: 'Incentivado Anual', value: reportTotals.annualTotal, color: 'text-[#0B4DA2]', sub: 'Balanço fiscal ano corrente' },
              { label: 'Acumulado Bruto', value: reportTotals.grandTotal, color: 'text-emerald-400', sub: 'Total confirmado' },
            ].map((item) => (
              <div key={item.label} className="p-4 bg-neutral-900 border border-neutral-800 rounded-xl">
                <span className="text-[10px] font-mono text-neutral-500 uppercase font-bold block mb-1">{item.label}</span>
                <div className={`text-xl font-bold font-mono ${item.color}`}>R$ {item.value.toLocaleString('pt-BR')}</div>
                <p className="text-[9px] text-neutral-500 mt-1">{item.sub}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="p-5 rounded-xl bg-neutral-900 border border-neutral-800 space-y-4">
              <div className="flex items-center space-x-2 pb-2 border-b border-neutral-850">
                ...
                <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-neutral-200">Ranking dos Maiores Doadores</h3>
              </div>
              <div className="space-y-2.5">
                {reportTotals.rankingSorted.length === 0 ? (
                  <p className="text-center text-xs p-6 text-neutral-500 italic">Nenhuma doação confirmada ainda.</p>
                ) : (
                  reportTotals.rankingSorted.map((g, idx) => (
                    <div key={idx} className="flex justify-between items-center p-2 rounded-lg bg-neutral-950 border border-neutral-900 text-xs">
                      <div className="flex items-center space-x-2.5">
                        <span className="w-5 h-5 rounded bg-neutral-900 border border-neutral-800 text-neutral-400 font-mono font-extrabold flex items-center justify-center text-[10px]">{idx + 1}</span>
                        <span className="font-sans font-bold text-neutral-200">{g.name}</span>
                      </div>
            ...
            <div className="p-5 rounded-xl bg-neutral-900 border border-neutral-800 space-y-4">
              <div className="flex items-center space-x-2 pb-2 border-b border-neutral-850">
                ...
                <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-neutral-200">Certidões e Isenção de Imposto</h3>
              </div>
              <div className="text-xs text-neutral-400 leading-relaxed space-y-3.5">
                ...
                <div className="p-3.5 rounded-lg bg-neutral-950 border border-neutral-800 font-mono text-[10px] text-amber-500/80 space-y-1">
                  <div className="flex justify-between"><span>CNPJ Associativo:</span><span className="text-neutral-300">12.345.678/0001-90</span></div>
                  <div className="flex justify-between"><span>Selo PRONAC Rouanet:</span><span className="text-neutral-300">#2415-MT-SUL</span></div>
                  <div className="flex justify-between"><span>Isenção ICMS:</span><span className="text-emerald-400 font-bold">CONCEDIDA</span></div>
                </div>
                <button onClick={() => alert('Extrato consolidado na pasta Biblioteca.')} className="w-full p-2 bg-neutral-800 hover:bg-neutral-750 text-neutral-300 text-[10.5px] font-mono rounded border border-neutral-750 transition-all cursor-pointer">
      ...
      {donationModalOpen && activeDonation !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 animate-fade-in">
          <form onSubmit={handleSaveDonation} className="w-full max-w-md bg-neutral-900 border border-neutral-800 rounded-xl overflow-hidden shadow-2xl">
            ...
            <div className="p-4 space-y-3.5">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-neutral-950 border border-neutral-800 text-neutral-100 p-2 text-xs rounded focus:outline-none"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-neutral-950 border border-neutral-800 text-emerald-400 p-2 text-xs rounded focus:outline-none font-mono font-bold"
                />
              </div>
            </div>

            <div className="bg-neutral-950 p-3 border-t border-neutral-800 flex justify-end space-x-2">
              <button type="button" onClick={() => setDonationModalOpen(false)} className="text-xs text-neutral-400 px-3 py-1 bg-neutral-900 rounded">Voltar</button>
      ...
      {supporterModalOpen && activeSupporter && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 animate-fade-in">
          <form onSubmit={handleSaveSupporter} className="w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-xl overflow-hidden shadow-2xl">
            <div className="bg-neutral-950 p-4 border-b border-neutral-800 flex justify-between items-center text-xs">
            ...
            <div className="p-4 space-y-3.5 max-h-[420px] overflow-y-auto">
              <div>
                <label className="block text-[10px] font-mono uppercase text-neutral-500 mb-1">Nome Fantasia</label>
                <input type="text" required value={activeSupporter.name || ''} onChange={(e) => setActiveSupporter({ ...activeSupporter, name: e.target.value })} className="w-full bg-neutral-950 border border-neutral-800 text-neutral-100 p-2 text-xs rounded focus:outline-none" />
              </div>
              <div>
                <label className="block text-[10px] font-mono uppercase text-neutral-500 mb-1">URL do Site</label>
                <input type="text" value={activeSupporter.siteUrl || ''} onChange={(e) => setActiveSupporter({ ...activeSupporter, siteUrl: e.target.value })} className="w-full bg-neutral-950 border border-neutral-800 text-neutral-100 p-2 text-xs rounded focus:outline-none font-mono" placeholder="https://suaempresa.com.br" />
              </div>
              <div>
                <label className="block text-[10px] font-mono uppercase text-neutral-500 mb-1">Texto Institucional</label>
                <textarea value={activeSupporter.description || ''} onChange={(e) => setActiveSupporter({ ...activeSupporter, description: e.target.value })} rows={2} className="w-full bg-neutral-950 border border-neutral-800 text-neutral-100 p-2 text-xs rounded focus:outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-mono uppercase text-neutral-500 mb-1">Nível de Patrocínio</label>
                  <select value={activeSupporter.sponsorshipLevel || 'silver'} onChange={(e) => setActiveSupporter({ ...activeSupporter, sponsorshipLevel: e.target.value as any })} className="w-full bg-neutral-950 border border-neutral-800 text-neutral-400 p-2 text-xs rounded">
                ...
                <div>
                  <label className="block text-[10px] font-mono uppercase text-neutral-500 mb-1">Segmento</label>
                  <input type="text" value={activeSupporter.category || ''} onChange={(e) => setActiveSupporter({ ...activeSupporter, category: e.target.value })} className="w-full bg-neutral-950 border border-neutral-800 text-neutral-100 p-2 text-xs rounded" placeholder="Ex: Indústria" />
                </div>
              </div>
              <div className="flex items-center space-x-2">
                <input type="checkbox" id="homeshow" checked={activeSupporter.highlightedOnHome || false} onChange={(e) => setActiveSupporter({ ...activeSupporter, highlightedOnHome: e.target.checked })} className="w-4 h-4 bg-neutral-950 border border-neutral-800" />
      ...
      {exportCsvLines && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-4 animate-fade-in">
          <div className="w-full max-w-xl bg-neutral-900 border border-neutral-800 rounded-xl overflow-hidden shadow-2xl">
            ...
            <div className="p-4 bg-neutral-950 font-mono text-[10px] text-neutral-400 overflow-auto max-h-72">
